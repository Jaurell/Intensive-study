...
for (var in dist2){
  ...
  s <- glm(get(var) ~ nsfs_c_mean_median + nsfs_r_mean_median + roleclarity_mean_median + from2016, data = wdata)
...
colnames(out) <- c('Dependent','N','F','P','R2','nsfs_c_median_beta','SE','t','OR','lower','upper','nsfs_r_median_beta','SE','t','OR','lower','upper','roleclarity_median_beta','SE','t','OR','lower','upper','from2016_beta','SE','t','OR','lower','upper')
